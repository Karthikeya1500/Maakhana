...
| UC2  | Select State          | Customer           | Choose an Indian state (e.g., Punjab, Tamil Nadu, West Bengal) to browse food from.           |
| UC3  | Browse Food by State  | Customer           | View food items from the selected state, listed by home chefs native to that state.           |
| UC4  | Browse Food Items     | Customer           | View available food items listed by home chefs, filtered by state.                            |
...
| UC11 | Search Food           | Customer           | Search food items by name, cuisine, or chef name across all states.                           |
| UC12 | Add Food Item         | HomeChef           | Create a new food listing with name, description, price, image, and state.                    |
...
| UC21 | Monitor Orders        | Admin              | View all platform orders with filters by status, state, date, and chef.                       |
| UC22 | View Platform Analytics | Admin            | Dashboard with platform-wide metrics: total orders, revenue, active chefs, state-wise stats.  |
...
| UC26 | Process Payment       | System             | Handle simulated payment processing during order placement.                                   |
| UC27 | Send Notification     | System             | Dispatch notifications for order updates, chef approvals, and status changes.                 |
| UC28 | Update Chef Profile   | HomeChef           | Update chef-specific profile info: specialties, state, bio.                                   |
